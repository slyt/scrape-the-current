...
import pandas as pd
from elements import visualize_top_songs_all_time, visualize_monthly_top_songs, visualize_yearly_top_songs# Import both functions
...
view_options = ["Top 89 Songs for All Time", 
                "Top 89 Songs Monthly",
                "Top 89 Songs Yearly"]
...
elif selected_view == "Top 89 Songs Yearly":
    st.markdown("# Top 89 Songs Yearly")
    # Year selection widget
    years = list(range(2006, 2025))
    selected_year = st.selectbox("Select Year", years)

    # Call visualization function with selection
    fig = visualize_yearly_top_songs.visualize_yearly_top_songs(selected_year, data_dir)
    st.plotly_chart(fig)
else:
